        ...
        private void CheckCurrentQueue() 
        {
            ...
            try
            {
                ...
                if (currentItem != null && currentItem.Initiated == true && currentItem.Completed == false)
                {
                    ...
                    {
                            if (currentItem is GameStateEventArgs)
                            {
                                var itm = currentItem as GameStateEventArgs;
                                if(!string.IsNullOrEmpty(itm.TargetClass))
                                {
                                    if (!string.IsNullOrEmpty(itm.MethodName))
                                        GD.Print(string.Format("Throwing unfinished Event! ({0} Seconds) , (Targetclassname: {1}, Method:{2})",TimeDiff.Seconds.ToString(),itm.TargetClass?.ToString(),itm.MethodName.ToString()));
                                    else 
                                         GD.Print(string.Format("Throwing unfinished Event! ({0} Seconds) , (Targetclassname: {1})",TimeDiff.Seconds.ToString(), itm.TargetClass?.ToString()));
                                }
                            }
                            else
